...
impl Msegmentation {
    pub fn get_segment_k(
        ...
        if segment_k_bn_rotated == BigInt::zero() {
            Scalar::<Secp256r1>::zero()
        } else {
            Scalar::<Secp256r1>::from(&segment_k_bn_rotated)
        }
    }
    //returns r_k,{D_k,E_k}
    pub fn encrypt_segment_k(
        secret: &Scalar<Secp256r1>,
        random: &Scalar<Secp256r1>,
        segment_size: &usize,
        k: u8,
        pub_ke_y: &Point<Secp256r1>,
        G: &Point<Secp256r1>,
    ) -> Helgamal {
        let segment_k = Msegmentation::get_segment_k(secret, segment_size, k);
        let E_k = G * random;
        let r_kY = pub_ke_y * random;
        if segment_k == Scalar::<Secp256r1>::zero() {
            let D_k = r_kY;
    ...
    pub fn assemble_fe(
        ...
        let seg_sum = segments_2n.iter().enumerate().fold(seg1, |acc, (i, s)| {
            if s.clone() == Scalar::<Secp256r1>::zero() {
                acc
            } else {
                let two_to_the_n = two.pow(*segment_size as u32);
                let two_to_the_n_shifted = two_to_the_n.shl(i * segment_size);
                let two_to_the_n_shifted_fe: Scalar<Secp256r1> =
                    Scalar::<Secp256r1>::from(&two_to_the_n_shifted);
    ...
    #[allow(clippy::ptr_arg)] // TODO: resolve this clippy warning (requires major version bump)
    pub fn assemble_ge(segments: &Vec<Point<Secp256r1>>, segment_size: &usize) -> Point<Secp256r1> {
        let two = BigInt::from(2);
        let mut segments_2n = segments.clone();
        let seg1 = segments_2n.remove(0);
        let seg_sum = segments_2n.iter().enumerate().fold(seg1, |acc, (i, s)| {
    ...
    pub fn decrypt_segment(
        DE: &Helgamal,
        G: &Point<Secp256r1>,
        private_key: &Scalar<Secp256r1>,
        limit: &u32,
        table: &[Point<Secp256r1>],
    ) -> Result<Scalar<Secp256r1>, Errors> {
        let mut result = None;

        let limit_plus_one = *limit + 1u32;
        let out_of_limit_fe: Scalar<Secp256r1> =
            Scalar::<Secp256r1>::from(&BigInt::from(limit_plus_one));
        let out_of_limit_ge: Point<Secp256r1> = G.clone() * &out_of_limit_fe;
        let yE = DE.E.clone() * private_key;
        // handling 0 segment
        let mut D_minus_yE: Point<Secp256r1> = out_of_limit_ge;
        if yE == DE.D.clone() {
            result = Some(());
        } else {
            D_minus_yE = &DE.D - &yE;
        ...
        match matched_value_index {
            Some(x) => Ok(Scalar::<Secp256r1>::from(&BigInt::from(x.0 as u32 + 1))),
            None => {
                if result.is_some() {
                    Ok(Scalar::<Secp256r1>::zero())
                } else {
                    Err(ErrorDecrypting)
                }
            }
    ...
    pub fn decrypt(
        ...
        let test_ge_table = (1..limit)
            .into_par_iter()
            .map(|i| {
                let test_fe = Scalar::<Secp256r1>::from(&BigInt::from(i));
                G * &test_fe
            })
        ...
        let vec_secret = (0..DE_vec.DE.len())
            ...
            .collect::<Vec<Result<Scalar<Secp256r1>, Errors>>>();
        let mut flag = true;
